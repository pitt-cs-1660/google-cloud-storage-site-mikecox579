...
function setupGame() {
  let divHtmlElement = document.getElementById("wrapper");
  ...
  for (let i = 0; i < myArray.length; i++) {
    ...
    imageElement.setAttribute("cardNumber", i);

    divElement.appendChild(imageElement);
    divHtmlElement.appendChild(divElement);
  }

  divHtmlElement = document.getElementById("numberOfTrials");

  let textElement = document.createElement("h2");
  textElement.textContent = trialsCounter;

  divHtmlElement.appendChild(textElement);
}
setupGame();
  ...
  if (numberOfCardsOpened === 0) {
    ...
    if (imageElement.src.includes("images/back_side.png")) {
      ...
      imageElement.src = `images/${
        images[Math.floor(myArray[cardNumber] / 2)]
      }.png`;
    ...
    if (imageElement.src.includes("images/back_side.png")) {
      trialsCounter += 1;
      let divHtmlElement = document.getElementById("numberOfTrials");

      let textElement = document.createElement("h2");
      textElement.textContent = trialsCounter;

      divHtmlElement.removeChild(divHtmlElement.firstElementChild);
